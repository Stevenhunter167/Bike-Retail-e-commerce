...
public class UpdateSecurePassword implements DB_params {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String loginUser = DB_params.username;
        String loginPasswd = DB_params.password;
        String loginUrl = "jdbc:mysql://localhost:3306/" + DB_params.db_name;
        Class.forName("com.mysql.jdbc.Driver").newInstance();
        Connection connection = DriverManager.getConnection(loginUrl, loginUser, loginPasswd);

        updateCustomer(connection);
        updateStaffs(connection);

//        connection.close();
    }

    private static void updateCustomer(Connection connection) throws Exception {
        ...
        String query = "SELECT customer_id, password from customers";
        ...
        while (rs.next()) {
            ...
            String updateQuery = String.format("UPDATE customers SET password='%s' WHERE customer_id=%s;", encryptedPassword, id);
...
//        connection.close();
    ...
    private static void updateStaffs(Connection connection) throws Exception {
        Statement statement = connection.createStatement();

        // change the customers table password column from VARCHAR(20) to VARCHAR(128)
        String alterQuery = "ALTER TABLE staffs MODIFY COLUMN password VARCHAR(128)";
        int alterResult = statement.executeUpdate(alterQuery);
        System.out.println("altering staffs table schema completed, " + alterResult + " rows affected");

        // get the ID and password for each customer
        String query = "SELECT staff_id, password from staffs";
        ...
        while (rs.next()) {
            ...
            String updateQuery = String.format("UPDATE staffs SET password='%s' WHERE staff_id=%s;", encryptedPassword, id);
            updateQueryList.add(updateQuery);
        }
        rs.close();
